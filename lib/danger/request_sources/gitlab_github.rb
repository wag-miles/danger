...
module Danger
  module RequestSources
    class GitLabGitHub < RequestSource
      include Danger::Helpers::CommentsHelper

      attr_accessor :pr_json, :pr_number, :issue_json, :support_tokenless_auth, :dismiss_out_of_range_messages
      ...
      def pr_diff
        @pr_diff ||= client.pull_request(ci_source.repo_slug, self.pr_number, accept: "application/vnd.github.v3.diff")
      end

      def review
        return @review unless @review.nil?
        begin
          @review = client.pull_request_reviews(ci_source.repo_slug, self.pr_number)
      ...
      def fetch_details
        self.pr_json = client.pull_requests(ci_source.repo_slug).find { |pr|
          pr.head.sha.eql? @commit_sha
        }

        if self.pr_json.nil?
          raise "Unable to find associated pull request on GitHub for SHA #{@commit_sha}"
        elsif self.pr_json["message"] == "Moved Permanently"
          raise "Repo moved or renamed, make sure to update the git remote".red
        end

        self.pr_number = self.pr_json[:url].split("/").last.to_i
      ...
      def issue_comments
        @comments ||= begin
          client.issue_comments(ci_source.repo_slug, self.pr_number)
      ...
      def update_pull_request!(warnings: [], errors: [], messages: [], markdowns: [], danger_id: "danger", new_comment: false, remove_previous_comments: false)
        ...
        if main_violations_sum.any?
          ...
          comment_result =
            if should_create_new_comment
              client.add_comment(ci_source.repo_slug, self.pr_number, body)
            else
      ...
      def submit_inline_comments!(warnings: [], errors: [], messages: [], markdowns: [], previous_violations: [], danger_id: "danger")
        ...
        pr_comments = client.pull_request_comments(ci_source.repo_slug, self.pr_number)
      ...
      def submit_inline_comments_for_kind!(kind, messages, diff_lines, danger_comments, previous_violations, danger_id: "danger")
        ...
        messages.reject do |m|
          ...
          if matching_comments.empty?
            begin
              client.create_pull_request_comment(ci_source.repo_slug, self.pr_number,
